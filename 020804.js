  ...
  try {
    ...
    for(o of $.openList){
      ...
      if(o.open != true && o.openCardUrl){
        ...
        await $.wait(parseInt(Math.random() * 1000 + 30, 10))
        if($.openCard == true){
          await joinShop()
          await takePostRequest('activity_load');
          await $.wait(parseInt(Math.random() * 1000 + 300, 10))
          // break
        }
        $.joinVenderId = ''
      }
    ...
    if(flag) await $.wait(parseInt(Math.random() * 1000 + 100, 10))
    if(guaopenwait){
      ...
      if($.index % 3 == 0) await $.wait(parseInt(Math.random() * 2000 + 600, 10))
    }
  } catch (e) {
    console.log(e)
  }
