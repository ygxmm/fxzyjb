...
const $ = new Env('jd_opencard_08.js')
...
!(async () => {
    ...
    let authorCodeList = [
        '6f053f44832c420d852c30e9a8874ce3',
    ];
...
async function run() {
    try {
        ...
        await $.wait(1500);
        $.openList = [139403,694930,725113,10080546,10517358,10537865,10622393,10924630,12150263];
...
async function joinShop() {
    ...
    let h5stLog = await getH5st($.joinVenderId);
    return new Promise(async resolve => {
...
async function getH5st(venderId) {
    let resp,h5stLog;
    while (!h5stLog){
        try {
            resp = await got.post("http:"+"//sign.tsukasa.pro:7412/"+"h"+"5"+"st/"+"get"+"By"+"Ven"+"derId",{
                json: {"venderId": venderId},
                timeout: 10000
            });
            h5stLog = resp.body
        }catch (e){
            let millisecond = parseInt(Math.random()*(2000-5000+1)+5000,10);
            $.log(`h5st异常！等待${millisecond}毫秒 `)
