    ...
    for scriptsName in tree:
        if scriptsName not in scriptsJson[Repo[0]]:
            ...
            try:
                ...
                if qlCron(GitRepo,"run",False):
                    ii = 0
                    while not os.path.exists(qlPath+scriptsName):
                        if ii>300:
                            log(f"等待添加脚本到本地文件夹超时>>{str(ii)}秒");break
                        time.sleep(1);ii+=1
                    ...
                    while len(rsp)==0:
                        if xx>120:
                            log(f"等待添加定时任务超时>>{str(xx)}秒");break
                        time.sleep(1);xx+=1
                        rsp = session.get(url=url,headers=qlHeader).json().get("data")
                    print(f"等待拉库成功：耗时{str(xx+ii)}秒")
                    if xx<130:
...
if 'GitRepoHost' in os.environ:
    ...
    if os.environ.get("opencardLoop")=="true":
        ...
        Delay = 16-2*len(RepoHost)
        times = 7200-len(RepoHost)*5-Delay
