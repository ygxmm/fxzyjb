...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/division/active/activity?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await superFans();
        ...
        if ($.secretPin) {
            ...
            if ($.activityContent) {
                ...
                if (!$.activityContent['followShop'].allStatus) {
                    await task('division/active/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}&shareUuid=${encodeURIComponent($.authorCode)}&taskType=1&taskValue=1`)
                } else {
                    $.log("已经关注过了\n")
                    await $.wait(2000);
                    // return
