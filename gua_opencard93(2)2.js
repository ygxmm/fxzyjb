...
All变量适用
————————————————
入口：[ 1.6～1.31 会员狂欢日 (https://lzdz1-isv.isvjcloud.com/dingzhi/majorsuit/memberday/activity?activityId=dz211250022468201&shareUuid=ca650078f09d43be8d535f36712ab9f2)]
...
process.env.guaopencard_draw93 = '3'
...
!(async () => {
  ...
  $.shareUuid = "c806b701a4c1498e9f4362830e2d7b2b"
...
async function run() {
  try {
    ...
    await $.wait(parseInt(Math.random() * 500, 10))
    if(flag) await $.wait(parseInt(Math.random() * 500, 10))
    if(guaopenwait){
      if($.index != cookiesArr.length){
        console.log(`等待${guaopenwait}秒`)
        await $.wait(parseInt(guaopenwait, 10) * 500)
      }
    }else{
